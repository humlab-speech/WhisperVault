...
def cmd_reload(args) -> int:
    ...
    payload: dict = {}
    for attr in (
        "model", "device", "compute_type", "language",
        "batch_size", "threads", "beam_size", "best_of",
        "vad_method", "vad_onset", "vad_offset", "chunk_size",
        "temperature", "initial_prompt", "hotwords",
    ):
        val = getattr(args, attr, None)
        if val is not None:
            payload[attr] = val
...
def _add_model_config_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--compute-type",   dest="compute_type", default=None,
...
def main() -> None:
    ...
    parser.add_argument(
        "--socket-dir", dest="socket_dir", default=DEFAULT_SOCKET_DIR,
    ...
    p_tx.add_argument("-o", "--output-dir", dest="output_dir", default=None,
                      help="Directory to write output files (default: ./output)")
    p_tx.add_argument("-f", "--output-format", dest="output_format", default="all",
    ...
    p_tx.add_argument("--interpolate-method", dest="interpolate_method",
                      default=None, choices=["nearest", "linear", "ignore"])
    p_tx.add_argument("--return-char-alignments", dest="return_char_alignments",
                      action="store_true", default=None)
    p_tx.add_argument("--speaker-embeddings", dest="speaker_embeddings",
                      action="store_true", default=None)
